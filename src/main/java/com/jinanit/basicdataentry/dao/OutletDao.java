...
public class OutletDao {
    ...
    public int saveOutlet(Outlet outlet) {
        String sInsertQuery = "insert into outlet(first_name, last_name, email, mobile_no, nid_no, business_name, business_address, business_category_id, trade_licence, trade_licence_photo, tin_no, password,entry_on) values ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, MD5(?),now())";
